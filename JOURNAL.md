...
const `darkerPixel` = 0.3 (default value so we can see the pixel dark on first pass)
...
2. apply multicolour or greyscale background colour to DIV on mousein
...
EVENT LISTENER for DIV `"screenPixels"`(MOUSE IN, FUNCTION(PARAMETER `event`))
   ...
   // Change background colour opacity property
    ...
    IF btnMulticolorToggle is not toggled
...
2. the toggle updates to switch back to greyscale on click.
...
5. the user clicks the greyscale toggle, ending colour mode and resetting toggle.
...
**FUNCTION** `determineDivHeight`(`gridSize`)  
**Uses:**  
`gridSize`  
**Creates / Returns:**  
...
**Does:**  
Loops through all child DIVs and removes them from DOM
